	...
	client.utils = {
		deleteIndice: async (indice) => {
			...
			if (!existsAlias) {
				...
				throw new Error(`unable-to-find-indice-for-${indice}`)
			}

			const promises = [
				`${indice}_1`,
				`${indice}_2`
			].map(async (index) => await client.indices.delete({ index }))

			if (options.log) options.log(`Delete ${indice} alias pointing to ${indice}_1 and ${indice}_2`)
		...
		generateRangeFilter: (field, values) => {
			...
			values.forEach((value) => {
				...
				range[field] = {
					gte: _.head(limits) ? _.min(limits) : undefined,
					lt: _.last(limits) ? _.max(limits) : undefined
				}
		...
		generateAggregation: (type, field, options) => {
			...
			const postFiltersFiltered = _.filter(options.post_filter || [], (filter) => {
				...
				if (type === 'range') return !_.get(_.get(filter, 'bool.should[0].range'), field)
			...
			aggregation[type] = {
				...
				percents: type === 'percentiles' ? options.percents : undefined
			}
